...
SAVE_MODEL = True
MODEL_PATH = "models/answerrnn3_att2.h5"
SHOW_PLOTS = False
...
if DATASET == Semeval2017:
    ...
    MAX_ANSWER_LENGTH = 27  # gl: real value = 27; safe value = 16; if change, change ENC_CANDIDATE_POOL_SIZE too or crash
    EMBEDDINGS_SIZE = 300
    ENC_CANDIDATE_POOL_SIZE = 12  # gl: 12 if MAX_ANSWER_LENGTH = 27; 6 if MAX_ANSWER_LENGTH = 16
    ATT_DIM = 50  # dim. of attentive output, should be a fraction of EMBEDDINGS_SIZE; best: 50
    BATCH_SIZE = 256
    ...
    ATT_DIM = 50  # dim. of attentive output
    BATCH_SIZE = 256
    PREDICT_BATCH_SIZE = 2048
    EPOCHS = 9
else:
...
if not SAVE_MODEL or not os.path.isfile(MODEL_PATH):
    ...
    embedded_document = layers.Embedding(np.shape(embedding_matrix)[0],
                                        EMBEDDINGS_SIZE,
    ...
    # gl: use this BLSTM to create context vectors (same number and dimensions of embedded words of document)
    context_document = layers.Bidirectional(
        layers.LSTM(int(EMBEDDINGS_SIZE/2),
                    activation='tanh',
                    recurrent_activation='tanh',
                    return_sequences=True))\
    ...
    att_document = layers.dot([context_document, embedded_document], 2)
    att_document = layers.Softmax()(att_document)
    print(att_document)
    att_document = layers.dot([att_document, embedded_document], 1)
    print(att_document)
    att_document = layers.Dense(int(ATT_DIM))(att_document)
    print(att_document)
    encoded_document = layers.Concatenate(axis=2)([embedded_document, att_document])
    print(encoded_document)

    #encoded_document = context_document
    # Size: 554
    ...
    embedded_candidate = layers.Embedding(np.shape(embedding_matrix)[0],
                                         EMBEDDINGS_SIZE,
                                         weights=[embedding_matrix],
    ...
    embedded_candidate = layers.Bidirectional(
        layers.LSTM(int(EMBEDDINGS_SIZE/2),
                    activation='tanh',
                    recurrent_activation='tanh',
                    return_sequences=True))\
    ...
    print(embedded_candidate)
    # attentive module for candidates

    att_candidate = layers.dot([context_document, embedded_candidate], 2)
    att_candidate = layers.Softmax()(att_candidate)
    print(att_candidate)
    att_candidate = layers.dot([att_candidate, embedded_document], 1)
    print(att_candidate)
    att_candidate = layers.Dense(int(ATT_DIM))(att_candidate)
    print(att_candidate)
    encoded_candidate = layers.Concatenate(axis=2)([embedded_candidate, att_candidate])
    #encoded_candidate = layers.Concatenate(axis=2)([context_candidate, att_candidate])
    print(encoded_candidate)

    '''
    att_candidate = layers.dot([context_candidate, embedded_candidate], 2)
    att_candidate = layers.Softmax()(att_candidate)
    print(att_candidate)
    att_candidate = layers.dot([att_candidate, embedded_candidate], 1)
    print(att_candidate)
    att_candidate = layers.Dense(int(ATT_DIM))(att_candidate)
    print(att_candidate)
    encoded_candidate = layers.Concatenate(axis=2)([embedded_candidate, att_candidate])
    ...
    encoded_candidate = layers.MaxPool1D(pool_size=ENC_CANDIDATE_POOL_SIZE)(encoded_candidate)  # gl: semeval2017: should be *2
    ...
    #print((Model([candidate, context_document], encoded_candidate)).summary())  # gl: was commented
    #print((Model()).summary())  # gl: was commented
