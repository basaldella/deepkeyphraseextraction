...
class Semeval2017(Dataset):
    ...
    def __load_documents(self, folder):
        ...
        for doc in os.listdir("%s/%s" % (self.path, folder)):
            if doc.endswith(".txt"):
                content = open(("%s/%s/%s" % (self.path, folder, doc)), "r",encoding="utf-8").read()
                documents[doc[:doc.find('.')]] = content
    ...
    def __load_answers(self, folder):
        ...
        for filename in file_list:
            if not filename.endswith(".ann"):
                continue
            file_anno = open(os.path.join("%s/%s" % (self.path, folder), filename), "rU",encoding="utf-8")
            file_text = open(os.path.join("%s/%s" % (self.path, folder), filename.replace(".ann", ".txt")),
                             "rU",encoding="utf-8")
